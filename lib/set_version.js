...
async function getIOSVersionInfo(versionText, config) {
    let versionInfo = {
...
async function setIosApplicationVersion(versionText, config) {
    ...
    const bundleVersion = `${version.build+1}`;
    if (version) {
        display("");
        ...
        try {
            ...
            fs.writeFileSync(path.join(appResourcesPath, paths.infoPlist), plist.build(plistInfo), "utf8");
            display(
...
async function setAndroidApplicationVersion(versionText, config) {
    ...
    if (versionCode) {
        versionCode++;
        display("");
        display(chalk.yellow("Android version1 info:"));
        display(version);
        ...
        try {
            ...
            if (
                ...
                fs.writeFileSync(
                    path.join(appResourcesPath, paths.androidManifest),
                    newAndroidManifest,
                    "utf8"
                );
                display(
